...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(data){
    const lowerCaseNames = data.map((item) => {return `${item.animal_name.toLowerCase()}`});
  ...
  */
  ...
  function lowPopulationAnimals(data){
    const lowPopulationAnimals = data.filter(item => {return item.population < 5});
    return lowPopulationAnimals;
  }
  // console.log('req 3:', lowPopulationAnimals(zooAnimals));
  ...
  */
  ...
  function USApop(data){
    const totalPopulation = data.reduce((acc, item) => {return acc + item.population}, 0);
    return totalPopulation;
  }
  // console.log('Req 4:', USApop(zooAnimals));
...
// already completed the array method callbacks using arrow functions
...
const stretchTest = new CubeMaker({
  length: 7,
  width: 7,
  height: 7
});

console.log('stretch volume:', stretchTest.volumeCube());
console.log('stretch surface area:', stretchTest.surfaceAreaCube());
